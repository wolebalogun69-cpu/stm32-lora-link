...
int main(void)
{
    ...
    uint32_t last_reported_overflow = 0U;
    ...
    while (1)
    {
        ...
        if (rx_overflow_count != last_reported_overflow)
        {
            char msg[80];
            last_reported_overflow = rx_overflow_count;
            sprintf(msg, "RX RING OVERFLOW count=%lu\r\n",
                    (unsigned long)last_reported_overflow);
            dbg_print(msg);
        }
