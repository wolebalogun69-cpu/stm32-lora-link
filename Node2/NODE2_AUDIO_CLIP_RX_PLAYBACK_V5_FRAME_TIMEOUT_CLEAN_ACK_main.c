...
#define UART_BYTE_TIMEOUT_MS     2000U
...
static volatile uint8_t audio_stop_requested = 0U;
...
static void stop_audio_playback_if_requested(void)
{
    if (audio_stop_requested)
    {
        ...
        dbg_print("AUDIO PLAYBACK DONE\r\n");
    }
}

static void reset_parser(uint8_t *state, uint16_t *index)
{
    *state = 0U;
    *index = 0U;
}

static uint8_t process_audio_payload(const uint8_t *payload,
                                     uint16_t len,
                                     AudioEvent *event,
                                     ...
                                     uint8_t *event_count)
{
    ...
    *event_count = 0U;
    ...
    if (type == AUDIO_MSG_START)
    {
        ...
        *event = AUDIO_EVENT_START;
        *event_offset = clip_len;
        *event_count = 0U;
    ...
    if (type == AUDIO_MSG_END)
    {
        ...
        if (!start_audio_playback(clip_len))
        {
            return 0U;
        }

        *event = AUDIO_EVENT_END;
        *event_offset = clip_len;
        *event_count = 0U;
...
static void print_audio_event(AudioEvent event, uint16_t offset, uint8_t count)
{
    char msg[100];

    if (event == AUDIO_EVENT_START)
    {
        sprintf(msg, "\r\nAUDIO START len=%lu\r\n", (unsigned long)offset);
        dbg_print(msg);
    }
    else if (event == AUDIO_EVENT_DATA)
    {
        sprintf(msg,
...
int main(void)
{
    ...
    dbg_print("\r\nNODE 2 AUDIO CLIP RX + PLAYBACK V5 FRAME TIMEOUT READY\r\n");
    ...
    while (1)
    {
        stop_audio_playback_if_requested();

        if (HAL_UART_Receive(&huart2, &b, 1U, UART_BYTE_TIMEOUT_MS) != HAL_OK)
        {
            if (state != 0U)
            {
                dbg_print("FRAME TIMEOUT - RESYNC\r\n");
                reset_parser(&state, &index);
            }
        ...
        switch (state)
        {
            ...
            case 8:
            {
                ...
                if (calc_checksum == rx_checksum)
                {
                    ...
                    {
                        AudioEvent event = AUDIO_EVENT_NONE;
                        uint16_t event_offset = 0U;
                        uint8_t event_count = 0U;

                        if (process_audio_payload(payload,
                                                  length,
                                                  &event,
                                                  &event_offset,
                                                  &event_count))
                        {
                            ...
                            send_ack(sequence);
                            print_audio_event(event, event_offset, event_count);
                        }
                        else
                        {
                ...
                else
                {
                    char msg[140];
                    sprintf(msg,
                            "\r\nCHECKSUM FAIL SEQ=%lu len=%lu calc=%lu rx=%lu\r\n\r\n",
                            (unsigned long)sequence,
                            (unsigned long)length,
